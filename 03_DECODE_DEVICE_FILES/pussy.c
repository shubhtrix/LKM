...
static ssize_t my_null_read(struct file *f, char __user *buf,
    ...
    // buf[0] = pussy_buffer[0];        // Good Practice to use copy_to_user instead
    
    if (copy_to_user (buf, pussy_buffer, 1) != 0) {
        return -EFAULT;
    } else {
        printk(KERN_INFO "READ Data from user buffer:: %c", buf[0]);
        return 1;
    }
}
static ssize_t my_null_write(struct file *f, const char __user *buf,
    ...
    //pussy_buffer[0] = buf[0];        // Good Practice to use copy_from_user instead
    
    if (copy_from_user (pussy_buffer,buf,1) !=0) {
        return -EFAULT;
    } else {
        printk(KERN_INFO "Buffer data provided by user :: %c", pussy_buffer[0]);
...
static int __init IN(void) {
	...
	if (device_create(cl, NULL, first, NULL, "pussy") == NULL) {
		class_destroy(cl);
		unregister_chrdev_region(first, 1);
		return -1;
	}
